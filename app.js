...
async function initSupabaseClient() {
  ...
  try {
    ...
    connectionError = err;
    console.warn('تعذّر تحميل عميل Supabase:', err);
    return null;
  }
}
...
async function initDB() {
  if (!supabase) supabase = await initSupabaseClient();
  ...
  // بعد تسجيل الدخول نتأكد أن سياسات RLS تسمح بقراءة بيانات الحساب.
  const { error } = await supabase.from('progress_accum').select('id').limit(1);
  if (error) {
...
function setAuthVisible(visible) {
  ...
  authScreen.setAttribute('aria-hidden', String(!visible));
  mainApp.classList.toggle('app-locked', visible);
}

async function requireAuthenticatedSession() {
  supabase = await initSupabaseClient();
  if (!supabase) throw new Error('تعذّر تحميل خدمة تسجيل الدخول. تحقق من الإنترنت ثم أعد المحاولة.');
  ...
  if (!session) {
    await new Promise((resolve) => {
      const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => {
        if (event === 'SIGNED_IN' && newSession) { listener.subscription.unsubscribe(); resolve(); }
      });
    });
  }
}
...
async function boot() {
  ...
  let initResult;
  try {
    initResult = await db.initDB();
  } catch (err) {
    showToast('تعذّر الوصول إلى قاعدة البيانات. راجع سياسات RLS: ' + err.message);
    updateConnectionBanner(false, err);
    return;
  }
  updateConnectionBanner(initResult.usingSupabase, initResult.error);
